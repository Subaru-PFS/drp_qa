...
from pfs.drp.stella.fitFluxReference import FilterCurve, TransmissionCurve
...
class FluxCalQaConnections(
    ...
    pfsCalibrated = PrerequisiteConnection(
        name="pfsCalibrated",
    ...
    fluxCalColorDiffPlot = OutputConnection(
        name="fluxCalColorDiffPlot",
        doc="Plot of the flux calibration color difference.",
        storageClass="Plot",
        dimensions=("instrument", "visit"),
    )
...
class FluxCalQaConfig(PipelineTaskConfig, pipelineConnections=FluxCalQaConnections):
    ...
    fakeJoffset = Field(dtype=float, default=0.054, doc="Fake J offset factor")
...
class FluxCalQaTask(PipelineTask):
    ...
    def run(self, pfsConfig: PfsConfig, pfsCalibrated: PfsCalibratedSpectra) -> Struct:
        ...
        pfsCalibrated : `pfs.datamodel.PfsCoadd`, optional
            Coadd spectra to use.
        ...
        if self.config.includeFakeJ:
            ...
            narrow_j_offset = self.config.fakeJoffset
