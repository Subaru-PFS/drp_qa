...
class FluxCalQA:
    ...
    def __init__(self, butler, verbose=False, isGen3=True, doAnonymize=True):
        ...
        if isGen3:
            ...
            self.butlerRoot = '%s/%s' % (
                butler.get_datastore_roots()['FileDatastore@<butlerRoot>'],
                butler.collections.defaults[0]
            )
        ...
        self.stellarMags = np.load(
            os.path.join(os.path.dirname(__file__), 'stellarMags.npy')
        ).transpose()  # columns are g_ps1, r_ps1, i_ps1, z_ps1, y_ps1, G, Bp, Rp
    ...
    def drawObsInfo(self, ax, pfsConfig, visit):
        ...
        info = self.getHeaderInfo(
            path,
            visit,
            [
                'OBJECT',
                'PROP-ID',
                'HIERARCH VERSION_DRP_STELLA',
                'HIERARCH VERSION_OBS_PFS',
                'HIERARCH VERSION_DATAMODEL',
                'W_PFDSNM'
            ]
        )
        ...
        ax.text(0.0, 0.90 - dy * 1,
                'pfsDesignId=%x, pfsDesignName=%s' % (pfsConfig.pfsDesignId, info['W_PFDSNM']),
                color='#777777')
        ax.text(0.0, 0.90 - dy * 2,
                'Exp_start=%s, Exptime=%3.0fsec' % (data[1], data[2]), color='#777777')
        ax.text(0.0, 0.90 - dy * 3,
                'Altitude=%2.2fdeg, Azimuth=%3.2fdeg, Insrot=%+2.2fdeg, PA=%+3.1fdeg'
                % (data[3], data[4], data[5], pfsConfig.posAng), color='#777777')
        ax.text(0.0, 0.90 - dy * 4,
                'R.A.=%sdeg, Dec.=%sdeg, Humidity=%s%%'
                % (data[6], data[7], data[8]), color='#777777')
        ax.text(0.0, 0.90 - dy * 5,
                f'collection={self.butlerRoot.replace("file:///work/datastore/", "")}, '
                f'calibRoot={self.butlerCalibRoot}, arms={pfsConfig.arms}',
                color='#AAAAAA')
        ax.text(0.0, 0.90 - dy * 6,
                f'DRP_STELLA={info["HIERARCH VERSION_DRP_STELLA"]}, '
                f'OBS_PFS={info["HIERARCH VERSION_OBS_PFS"]}, '
                f'DATAMODEL={info["HIERARCH VERSION_DATAMODEL"]}',
                color='#AAAAAA')
    ...
    def drawAG(self, ax, visit):
        ...
        ax.text(xmin + (xmax - xmin) * 0.25, 1.2,
                'transparency = %1.2f +/- %1.2f'
                % (quartile1[1], (quartile1[2] - quartile1[0]) / 1.349),
                color='blue', ha='center'
                )
        ax.text(xmax - (xmax - xmin) * 0.25, 1.2,
                'seeing = %1.2f +/- %1.2f arcsec'
                % (quartile2[1], (quartile2[2] - quartile2[0]) / 1.349),
                color='red', ha='center'
                )
    ...
    def drawPfsMerged(self, ax, pfsConfig, visit, SNMinWave=840., SNMaxWave=875.):
        # originally 400-550
        ...
        for i in range(len(pfsConfig)):
            ...
            if pfsConfig.targetType[i] != datamodel.TargetType.FLUXSTD:
                # this can be modified to include, e.g., SCIENCE
                continue
            ...
            mag[i] = self.synthmag.getMag(
                merged.wavelength[index][good] * 10,
                merged.flux[index][good] / merged.norm[index][good] / self.exptime,
                filterName=self.refFilterFile,
                verbose=False,
                fnu=True,
                midres=midres
            ) - (-2.5 * np.log10(pfsConfig.psfFlux[i][self.psfFluxIndex]) + 31.4)
            tmp1 = self.synthmag.getMag(
                merged.wavelength[index][good] * 10,
                merged.flux[index][good] / merged.norm[index][good] / self.exptime,
                filterName=self.synthmag.filters[self.psfColorIndex[0]],
                verbose=False,
                fnu=True,
                midres=midres
            ) - (-2.5 * np.log10(pfsConfig.psfFlux[i][self.psfColorIndex[0]]) + 31.4)
            tmp2 = self.synthmag.getMag(
                merged.wavelength[index][good] * 10,
                merged.flux[index][good] / merged.norm[index][good] / self.exptime,
                filterName=self.synthmag.filters[self.psfColorIndex[1]],
                verbose=False,
                fnu=True,
                midres=midres
            ) - (-2.5 * np.log10(pfsConfig.psfFlux[i][self.psfColorIndex[1]]) + 31.4)
            color[i] = tmp1 - tmp2
            ...
            index2 = np.where(
                (merged.wavelength[index] > SNMinWave) &
                (merged.wavelength[index] < SNMaxWave) &
                good
            )[0]
            flux_data = merged.flux[index][index2]
            covar_data = merged.covar[index][0][index2]
            valid_mask = (covar_data > 0) & np.isfinite(covar_data)
            sn_values = np.full_like(flux_data, np.nan)
            sn_values[valid_mask] = flux_data[valid_mask] / np.sqrt(covar_data[valid_mask])
            SN[i] = np.nanmedian(sn_values)
        scatter = np.nanpercentile(mag, (25, 50, 75))
        ax.set_title(
            '%s, median=%2.2f, scatter=%1.3fmag' % (
                self.synthmag.filters[self.psfFluxIndex],
                scatter[1],
                (scatter[2] - scatter[0]) / 1.35
            ),
            fontsize=10
        )
        mag -= scatter[1]  # subtract the median
        ax.text(pfsConfig.raBoresight, pfsConfig.decBoresight + 0.7,
                'the global median is subtracted', horizontalalignment='center',
                fontsize=7)
            ...
            if np.isnan(mag[i]):
                ...
                if SN[i] < 10:
                    circle = patches.Circle(
                        (pfsConfig.ra[i], pfsConfig.dec[i]), 0.035,
                ...
                sc = ax.scatter(
                    pfsConfig.ra[i], pfsConfig.dec[i], marker=self.marker[spectrograph],
                    vmin=-0.5, vmax=0.5, s=10, c=mag[i], cmap=cm.coolwarm)
        ...
        # pfi -> sky: (PA - 90)
        length = 0.1
        angle = np.deg2rad(270. + (self.insrot + 90.) + (pfsConfig.posAng - 90.))
        # 270 deg is because gravity is downwards
        ...
        # minus is because R.A. increases towards left
    ...
    def drawPfsMergedColor(self, ax, pfsConfig, color, SN):
        ...
        ax.set_title(
            '%s - %s, median=%2.2f, scatter=%1.3fmag' % (
                self.synthmag.filters[self.psfColorIndex[0]],
                self.synthmag.filters[self.psfColorIndex[1]],
                scatter[1], (scatter[2]-scatter[0])/1.35),
            fontsize=9)
        color -= scatter[1]  # subtract the median
        ax.text(pfsConfig.raBoresight, pfsConfig.decBoresight + 0.7,
                'the global median is subtracted', horizontalalignment='center', fontsize=7)

        # store the results
        self.results['mergedColorFilters'].append('%s-%s' % (
            self.synthmag.filters[self.psfColorIndex[0]],
            self.synthmag.filters[self.psfColorIndex[1]]))
        ...
        for i in range(len(pfsConfig)):
            ...
            if np.isnan(color[i]):
                ...
                sc = ax.scatter(
                    pfsConfig.ra[i], pfsConfig.dec[i], marker=self.marker[spectrograph],
                    vmin=-0.2, vmax=0.2, s=10, c=color[i], cmap=cm.coolwarm)
        ...
        # 270 deg is because gravity is downwards
        ax.arrow(
            pfsConfig.raBoresight + dra * 0.75, pfsConfig.decBoresight - ddec * 0.75,
            length * np.cos(angle), length * np.sin(angle), width=0.02
        )  # minus is because R.A. increases towards left
    ...
    # -------------------------------------------------------------
    # show reference magnitude distribution of FLUXSTDs
    ...
    def drawFluxCalNorm(self, ax, pfsConfig, fluxCal):
        ...
        fluxNorm = fluxCal.evaluate(
            wavelengths=wavelengths, positions=positions,
            fiberIds=None).values.reshape(1, len(pfsConfig))[0]
        ...
        pcm = ax.scatter(
            pfsConfig.ra, pfsConfig.dec, marker='o', vmin=0.5, vmax=1.5,
            s=3, c=fluxNorm, cmap=cm.coolwarm_r)
        plt.colorbar(pcm, ax=ax)

        ax.set_xlabel('R.A. [deg]')
        ax.set_ylabel('Dec. [deg]')
        ax.set_title('median=%1.2e, fractional scatter=%1.2f%%' % (
            median, (scatter[2]-scatter[0])/1.35*100.), fontsize=10)
    ...
    def drawSingle(self, ax, pfsConfig, visit):
        ...
        for i in range(len(pfsConfig)):
            ...
            for j, filter in enumerate(self.synthmag.filters):
                mag[j][i] = self.synthmag.getMag(
                    single.wavelength[good] * 10, single.flux[good], filterName=filter,
                    verbose=False, fnu=True, midres=midres, medianFilterFine=True) - ref[j]
        ...
        for i in range(len(pfsConfig)):
            ...
            spectrograph = spectrographFromFiberId(pfsConfig.fiberId[i])
            if np.isnan(mag[self.psfFluxIndex][i]):
                ax.plot(
                    pfsConfig.ra[i], pfsConfig.dec[i], marker=self.marker[spectrograph],
                ...
                sc = ax.scatter(
                    pfsConfig.ra[i], pfsConfig.dec[i], marker=self.marker[spectrograph],
                    vmin=-0.1, vmax=0.1, s=10, c=mag[self.psfFluxIndex][i], cmap=cm.coolwarm)

        plt.colorbar(sc, ax=ax)
        ax.set_xlabel('R.A. [deg]')
        ax.set_ylabel('Dec. [deg]')
        ax.text(
            pfsConfig.raBoresight, pfsConfig.decBoresight + 0.7,
            'mag difference in %s' % self.synthmag.filters[self.psfFluxIndex],
            horizontalalignment='center', fontsize=9)

        scatter = np.nanpercentile(mag[self.psfFluxIndex], (25, 50, 75))
        ax.set_title('median=%1.3fmag, scatter=%1.3fmag' % (
            scatter[1], (scatter[2] - scatter[0]) / 1.35), fontsize=10)
    ...
    def drawSingleColor(self, ax, pfsConfig, mag):
        ...
        for i in range(len(pfsConfig)):
            ...
            spectrograph = spectrographFromFiberId(pfsConfig.fiberId[i])
            if (
                np.isnan(mag[self.psfColorIndex[0]][i]) or np.isnan(mag[self.psfColorIndex[1]][i])
            ):
                ax.plot(
                    pfsConfig.ra[i], pfsConfig.dec[i],
                    marker=self.marker[spectrograph], color='green', markersize=1
                )
                # print('debug', mag[self.psfColorIndex[0]][i], mag[self.psfColorIndex[1]][i])
            else:
                sc = ax.scatter(
                    pfsConfig.ra[i], pfsConfig.dec[i], marker=self.marker[spectrograph],
                    vmin=-0.1, vmax=0.1, s=10,
                    c=mag[self.psfColorIndex[0]][i] - mag[self.psfColorIndex[1]][i], cmap=cm.coolwarm
                )
        ...
        ax.set_ylabel('Dec. [deg]')
        ax.text(pfsConfig.raBoresight, pfsConfig.decBoresight + 0.7,
                '%s-%s color difference' % (
                    self.synthmag.filters[self.psfColorIndex[0]],
                    self.synthmag.filters[self.psfColorIndex[1]]
                ), horizontalalignment='center', fontsize=9)

        color = mag[self.psfColorIndex[0]] - mag[self.psfColorIndex[1]]
        index = np.where(~np.isnan(color))[0]
        scatter = np.nanpercentile(color[index], (25, 50, 75))
        ax.set_title(
            'median=%1.3fmag, scatter=%1.3fmag' % (scatter[1], (scatter[2]-scatter[0])/1.35), fontsize=10
        )
    ...
    def caption(self, saveFigDir=None):
        ...
        self.drawCaption(
            ax,
            "S/N of FLUXSTDs from pfsMerged\n"
        ...
        self.drawCaption(
            ax,
            "color-color diagram. The gray points are\n"
        ...
        plt.subplots_adjust(hspace=0.4, wspace=0.4, top=0.8, bottom=0.2)

        if saveFigDir is None:
            fig.show()
        else:
    ...
    def setFilterIndices(self, pfsConfig):
        ...
        if filterNums[0] == 5:          # this is ps1 - filters are in the order of grizy
            ...
            # refers to PS1 g-band and z - the filter set is hard-coded but should be made configurable
            self.psfColorIndex[0] = filterNames.index('g_ps1')
            # refers to PS1 g-band and z - the filter set is hard-coded but should be made configurable
            self.psfColorIndex[1] = filterNames.index('i_ps1')
            self.refCat = 'PS1'
        ...
        if (self.psfFluxIndex < 0) and (filterNums[0] > 0):
            ...
            except Exception:  # if the i-band is not available skip this visit
                self.psfFluxIndex = -1
                print('no appropriate filter set is found for fluxCalQA. Skipping this visit.')
                return False
            ...
            try:
                ...
                self.psfColorIndex[0] = filterNames.index('g_ps1')
                # refers to PS1 g-band and z - the filter set is hard-coded but should be made configurable
                self.psfColorIndex[1] = filterNames.index('z_ps1')
            except Exception:  # if no filter set is found for color, ignore color plots
    ...
    def drawFluxCalVector(self, ax, pfsConfig, fluxCal):
        ...
        percentile = np.nanpercentile(fluxNorm, (16, 50, 84), axis=0)
        percentile = np.where(percentile > 0, percentile, np.nan)
        percentile = np.log10(percentile)
    ...
    def drawFluxCalVectorColor(self, ax, pfsConfig, fluxCal):
        ...
        pcm = plt.scatter(
            pfsConfig.ra, pfsConfig.dec, marker='o', vmin=0.8, vmax=1.2, s=3, c=ratio, cmap=cm.coolwarm_r
        )
        plt.colorbar(pcm, ax=ax)
        ...
        ax.set_title('median=%1.2e, fractional scatter=%1.2f%%' % (
            median, (scatter[2]-scatter[0])/1.35*100.), fontsize=10)
        ax.text(pfsConfig.raBoresight, pfsConfig.decBoresight + 0.7,
                'ratio between %3.1fnm and %3.1fnm' % (
                    self.fluxCalVectorColorWave[0], self.fluxCalVectorColorWave[1]),
                horizontalalignment='center', fontsize=9)
    ...
    def drawTwoColorDiagram(self, ax, pfsConfig, SN, mag, index1, index2, index3):
        ...
        # plot PS1 sources
        ax.plot(
            self.stellarMags[index2] - self.stellarMags[index3],
            self.stellarMags[index1] - self.stellarMags[index2],
            '.', color='gray', alpha=1, markersize=0.3)
        ...
        coeffs = (
            (-12.74432612, 5.72884386, 1.35199207, 0.12986592),
            (73.19247901, -5.73469827, 1.42925779, 0.09580749),
            (8.85481888e+01, -6.46929173e+00, 1.10589459e+00, 6.33426991e-04)
        )
        ...
        for i in range(len(pfsConfig)):
            ...
            ax.plot(
                pfsMag[index2] - pfsMag[index3], pfsMag[index1] - pfsMag[index2],
                '.', color='black', alpha=0.7)

            # to measure the color scatter
            diffRef[diffNum] = (
                (ref[index1] - ref[index2]) -
                np.poly1d(coeffs[index1])(ref[index2] - ref[index3])
            )
            diffPfs[diffNum] = (
                (pfsMag[index1] - pfsMag[index2]) -
                np.poly1d(coeffs[index1])(pfsMag[index2] - pfsMag[index3])
            )
            diffNum += 1
        ...
        ax.set_title(
            'sigmaRef = %1.3f, sigmaPFS = %1.3f' % ((percentileRef[1] - percentileRef[0]) / 1.35, sigmaPfs))

        label = ['stellarSequenceSigmaGRI', 'stellarSequenceSigmaRIZ', 'stellarSequenceSigmaIZY']
        self.results[label[index1]].append(sigmaPfs)

    # -------------------------------------------------------------
    # initialize the result instance variables
    # - there is probably a better way to do this; I am not a python expert
    def initializeResults(self):
        self.results = {}
        for name in [
            'version', 'collection', 'visit', 'transparencyMedian',
            'transparencySigma', 'seeingMedian', 'seeingSigma',
            'mergedFilter', 'mergedMedian', 'mergedSigma',
            'mergedSNMedian', 'mergedSNSigma', 'imagMedian',
            'imagSigma', 'mergedColorFilters', 'mergedColorMedian',
            'mergedColorSigma', 'singleMedian', 'singleSigma',
            'singleColorMedian', 'singleColorSigma',
            'stellarSequenceSigmaGRI', 'stellarSequenceSigmaRIZ',
            'stellarSequenceSigmaIZY'
        ]:
            self.results[name] = []
    ...
    def plot(self, visits, saveFigDir=None, skipFluxCal=False):
        ...
        for visit in visits:
            ...
            self.mags_cat = np.append(
                self.mags_cat,
                np.array([
                    -2.5 * np.log10(flx[self.psfFluxIndex]) + 31.4
                    if tt == datamodel.TargetType.FLUXSTD
                    else np.nan
                    for tt, flx in zip(pfsConfig.targetType, pfsConfig.psfFlux)
                ])
            )
...
def main():
    ...
    if args.saveData:
        ...
        df2 = pd.DataFrame({
            "visit": fluxcalqa.visits,
            "fiberId": fluxcalqa.fids,
            "mag": fluxcalqa.mags,
            "color": fluxcalqa.cols,
            "mag_cat": fluxcalqa.mags_cat,
            "SNR": fluxcalqa.snrs
        })
