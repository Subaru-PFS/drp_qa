...
class FluxCalQA:
    ...
    def __init__(self, butler, verbose=False, isGen3=True, doAnonymize=True):
        ...
        if isGen3:
            ...
            self.butlerRoot = "{}/{}".format(
                butler.get_datastore_roots()["FileDatastore@<butlerRoot>"], butler.collections.defaults[0]
            )
        ...
        self.fluxCalVectorColorWave = [470.0, 866.0]  # wavelengths to compute fluxCalVector ratios
            ...
            for arm in ["b", "r", "n", "m"]:
                try:
                    calexp = self.butler.get(
                        "calexp", {"visit": visit, "spectrograph": spectrograph, "arm": arm}
                    )
    ...
    def drawObsInfo(self, ax, pfsConfig, visit):
        ...
        ax.text(
            0.0,
            0.90 - dy * 1,
            "pfsDesignId={:x}, pfsDesignName={}".format(pfsConfig.pfsDesignId, info["W_PFDSNM"]),
            color="#777777",
        )
        ax.text(0.0, 0.90 - dy * 2, f"Exp_start={data[1]}, Exptime={data[2]:3.0f}sec", color="#777777")
        ax.text(
            0.0,
            0.90 - dy * 3,
            f"Altitude={data[3]:2.2f}deg, Azimuth={data[4]:3.2f}deg, Insrot={data[5]:+2.2f}deg, PA={pfsConfig.posAng:+3.1f}deg",
            color="#777777",
        )
        ax.text(
            0.0, 0.90 - dy * 4, f"R.A.={data[6]}deg, Dec.={data[7]}deg, Humidity={data[8]}%", color="#777777"
        )
        ax.text(
            0.0,
            0.90 - dy * 5,
    ...
    def drawAG(self, ax, visit):
        ...
        ax.text(
            xmin + (xmax - xmin) * 0.25,
            1.2,
            f"transparency = {quartile1[1]:1.2f} +/- {(quartile1[2] - quartile1[0]) / 1.349:1.2f}",
            color="blue",
            ha="center",
        )
        ax.text(
            xmax - (xmax - xmin) * 0.25,
            1.2,
            f"seeing = {quartile2[1]:1.2f} +/- {(quartile2[2] - quartile2[0]) / 1.349:1.2f} arcsec",
            color="red",
            ha="center",
        )
    ...
    def drawPfsMerged(self, ax, pfsConfig, visit, SNMinWave=840.0, SNMaxWave=875.0):
        # originally 400-550
        ...
        for i in range(len(pfsConfig)):
            ...
            if np.isnan(mag[i]):
                mag[i] = -99.0  # just to differentiate from non-FLUXSTDs

            index2 = np.where(
                (merged.wavelength[index] > SNMinWave) & (merged.wavelength[index] < SNMaxWave) & good
            )[0]
        ...
        ax.set_title(
            f"{self.synthmag.filters[self.psfFluxIndex]}, median={scatter[1]:2.2f}, scatter={(scatter[2] - scatter[0]) / 1.35:1.3f}mag",
            fontsize=10,
        )
        ...
        ax.text(
            ...
            "the global median is subtracted",
        ...
        for i in range(len(pfsConfig)):
            ...
            if np.isnan(mag[i]):
                ...
                sc = ax.scatter(
                    pfsConfig.ra[i],
                    pfsConfig.dec[i],
                    marker=self.marker[spectrograph],
                    vmin=-0.5,
                    vmax=0.5,
                    s=10,
                    c=mag[i],
        ...
        # pfi -> sky: (PA - 90)
        length = 0.1
        angle = np.deg2rad(270.0 + (self.insrot + 90.0) + (pfsConfig.posAng - 90.0))
        # 270 deg is because gravity is downwards
        ax.arrow(
            pfsConfig.raBoresight + dra * 0.75,
            pfsConfig.decBoresight - ddec * 0.75,
            length * np.cos(angle),
            length * np.sin(angle),
            width=0.02,
        )
    ...
    def drawPfsMergedColor(self, ax, pfsConfig, color, SN):
        ...
        # store the results
        self.results["mergedColorFilters"].append(
    ...
    def drawMergedSN(self, ax, SN, SNMinWave, SNMaxWave):
        ax.hist(SN, bins=20)
        ax.set_xlabel(f"SN at {SNMinWave:3.1f} - {SNMaxWave:3.1f} nm")
        ax.set_ylabel("N")
        scatter = np.nanpercentile(SN, (25, 50, 75))
        ax.set_title(
            f"median={scatter[1]:2.2f}, scatter={(scatter[2] - scatter[0]) / 1.35:1.3f}", fontsize=10
        )
    ...
    def drawFluxCalNorm(self, ax, pfsConfig, fluxCal):
        ...
        fluxNorm = fluxCal.evaluate(
            wavelengths=wavelengths, positions=positions, fiberIds=None
        ).values.reshape(1, len(pfsConfig))[0]
        ...
        pcm = ax.scatter(
            pfsConfig.ra, pfsConfig.dec, marker="o", vmin=0.5, vmax=1.5, s=3, c=fluxNorm, cmap=cm.coolwarm_r
        )
        ...
        ax.set_ylabel("Dec. [deg]")
        ax.set_title(
            f"median={median:1.2e}, fractional scatter={(scatter[2] - scatter[0]) / 1.35 * 100.0:1.2f}%",
            fontsize=10,
        )
    ...
    def drawSingle(self, ax, pfsConfig, visit):
        ...
        for i in range(len(pfsConfig)):
            ...
            for j, filter in enumerate(self.synthmag.filters):
                mag[j][i] = (
                    self.synthmag.getMag(
                        single.wavelength[good] * 10,
                        single.flux[good],
                        filterName=filter,
                        verbose=False,
                        fnu=True,
                        midres=midres,
                        medianFilterFine=True,
                    )
                    - ref[j]
                )
        ...
        ax.text(
            pfsConfig.raBoresight,
            pfsConfig.decBoresight + 0.7,
            f"mag difference in {self.synthmag.filters[self.psfFluxIndex]}",
            horizontalalignment="center",
            fontsize=9,
        )
        ...
        ax.set_title(
            f"median={scatter[1]:1.3f}mag, scatter={(scatter[2] - scatter[0]) / 1.35:1.3f}mag", fontsize=10
        )
    ...
    def drawSingleColor(self, ax, pfsConfig, mag):
        for i in range(len(pfsConfig)):
            ...
            spectrograph = spectrographFromFiberId(pfsConfig.fiberId[i])
            if np.isnan(mag[self.psfColorIndex[0]][i]) or np.isnan(mag[self.psfColorIndex[1]][i]):
                ax.plot(
                    pfsConfig.ra[i],
                    pfsConfig.dec[i],
                    marker=self.marker[spectrograph],
                ...
                sc = ax.scatter(
                    ...
                    vmax=0.1,
                    s=10,
                    c=mag[self.psfColorIndex[0]][i] - mag[self.psfColorIndex[1]][i],
                    cmap=cm.coolwarm,
                )
    ...
    def caption(self, saveFigDir=None):
        ...
        if saveFigDir is None:
            ...
            fig.savefig(f"{saveFigDir}/fluxCalQA_caption.png", bbox_inches="tight")
    ...
    def drawFluxCalVector(self, ax, pfsConfig, fluxCal):
        waveGrid = np.arange(380.0, 1261.0, 1)
    ...
    def drawFluxCalVectorColor(self, ax, pfsConfig, fluxCal):
        ...
        ax.text(
            pfsConfig.raBoresight,
            pfsConfig.decBoresight + 0.7,
            f"ratio between {self.fluxCalVectorColorWave[0]:3.1f}nm and {self.fluxCalVectorColorWave[1]:3.1f}nm",
            horizontalalignment="center",
            fontsize=9,
        )
        ...
        for target in calibrated:
            if target.objId == pfsConfig.objId[index] and target.catId == pfsConfig.catId[index]:
                ...
                if (self.refCat == "PS1") and (self.flagNoColor is False):
                    wave = [479.65, 616.09, 750.62, 865.83, 961.15, 1150.0]
                ...
                ax.plot(wave, np.power(10.0, -0.4 * (pfsMags - 31.4)) / median, "o", color="blue")
    ...
    def drawTwoColorDiagram(self, ax, pfsConfig, SN, mag, index1, index2, index3):
        ...
        coeffs = (
            ...
            (8.85481888e01, -6.46929173e00, 1.10589459e00, 6.33426991e-04),
        )
        ...
        for i in range(len(pfsConfig)):
            ...
            diffRef[diffNum] = (ref[index1] - ref[index2]) - np.poly1d(coeffs[index1])(
                ref[index2] - ref[index3]
            )
            diffPfs[diffNum] = (pfsMag[index1] - pfsMag[index2]) - np.poly1d(coeffs[index1])(
                pfsMag[index2] - pfsMag[index3]
            )
            diffNum += 1
        ...
        ax.set_title(
            f"sigmaRef = {(percentileRef[1] - percentileRef[0]) / 1.35:1.3f}, sigmaPFS = {sigmaPfs:1.3f}"
        )
    ...
    def initializeResults(self):
        ...
        for name in [
            ...
            "singleColorMedian",
            "singleColorSigma",
            "stellarSequenceSigmaGRI",
            "stellarSequenceSigmaRIZ",
        ...
        for visit in visits:
            ...
            self.mags_cat = np.append(
                ...
                np.array(
                    [
                        ...
                        for tt, flx in zip(pfsConfig.targetType, pfsConfig.psfFlux, strict=False)
                    ]
                ),
            )
            ...
            if skipFluxCal:
                if saveFigDir is None:
                    ...
                    fig.savefig(f"{saveFigDir}/fluxCalQA_v{visit}.png", bbox_inches="tight")
                if self.verbose:
                    print("all done")
                continue
