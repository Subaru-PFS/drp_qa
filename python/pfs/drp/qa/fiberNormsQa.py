...
class _ThreadProcArgs(typing.TypedDict):
    ...
    input : `FiberNormsQaInput`
        Input to ``FiberNormsQa``.
    config : `FiberNormsQaConfig`
        Config of ``FiberNormsQa``.
    log : `logging.Logger`
        Parent logger.
    """
...
@dataclasses.dataclass
class FiberNormsQaConfig:
    """Config for `FiberNormsQa`."""
    ...
    random_seed: str = dataclasses.field(
        default="FiberNormsQa random seed",
...
class FiberNormsQa:
    ...
    def run(self) -> None:
        ...
        figure_path = os.path.join(
            self.output, f"fiberNormsQa_{self.arm}_{self.visit}_over_{self.ref_visit}.png"
        )
    ...
    def get_stat(self) -> FiberNormsQaStat:
        """Get statistics.

        Returns
        -------
        stat : `FiberNormsQaStat`
            statistics.
        """
    ...
    @staticmethod
    def _validate_input(input: FiberNormsQaInput) -> None:
