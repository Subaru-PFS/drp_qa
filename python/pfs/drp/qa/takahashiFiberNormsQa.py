...
def main() -> None:
    ...
    This QA was developed to monitor the fiber-to-fiber throughput issues per visit.
    Index-matching gel has been applied for all fibers by run20; however,
    it is still crucial to monitor whether such variations occur with MTP/fiber units.
    We take a quartz flux ratio, and show the uniformities of target quartz in the focal plane
    and how the spectra change relative to those of reference quartz.
    """
    ...
    if error_count == 0:
        log.info("%d jobs succeeded.", success_count)
    else:
        log.warning("%d jobs succeeded / %d jobs failed.", success_count, error_count)
...
def _main_threadproc(args: _ThreadProcArgs) -> _ThreadProcStatus:
    ...
    try:
        log = log.getChild(f"pid={os.getpid()}")
